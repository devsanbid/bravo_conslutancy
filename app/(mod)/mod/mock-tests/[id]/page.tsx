...
import { ArrowLeft, PlusCircle, Edit, Trash2, FileText, Headphones, PenTool, Mic } from "lucide-react";
  ...
  return (
    <div className="container mx-auto py-6">
      <div className="flex items-center mb-6">
        <Button variant="ghost" onClick={() => router.push("/mod/mock-tests")} className="mr-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              <div>
                <span className="font-semibold">Duration:</span> {mockTest.duration} minutes
              </div>
              <div>
                <span className="font-semibold">Total Marks:</span> {mockTest.totalMarks}
              </div>
              <div>
                <span className="font-semibold">Status:</span>{" "}
                <span className={mockTest.isActive ? "text-green-600" : "text-red-600"}>
          ...
          <CardFooter className="flex justify-between">
            ...
            <Button
              variant="outline"
              onClick={() => router.push(`/mod/mock-tests/${params.id}/preview`)}
            >
              Preview Test
            </Button>
          </CardFooter>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={() => router.push(`/mod/mock-tests/${params.id}/questions/create`)}>
          <PlusCircle className="mr-2 h-4 w-4" /> Add Question
        </Button>
      </div>

      <Tabs defaultValue="multiple_choice" onValueChange={(value) => setActiveTab(value as QuestionType)}>
        ...
        <TabsContent value={activeTab}>
          <Card>
            <CardHeader>
              <CardTitle>{activeTab.split("_").map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ")} Questions</CardTitle>
              <CardDescription>
                Manage {activeTab.split("_").map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ")} questions for this test.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {loading ? (
                ...
                <div className="text-center py-8">
                  <p className="text-muted-foreground">No {activeTab.split("_").map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ")} questions found.</p>
                  <Button 
                    variant="outline" 
                    className="mt-4"
                    onClick={() => router.push(`/mod/mock-tests/${params.id}/questions/create?type=${activeTab}`)}
                  >
                    Add your first {activeTab.split("_").map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ")} question
                  </Button>
                </div>
              ) : (
                      ...
                      <Card key={question.id} className="relative">
                        ...
                        <CardContent>
                          ...
                          {question.questionType === "fill_blank" && (
                            <div>
                              <p className="font-medium">Answer: {Array.isArray(question.correctAnswer) 
                          ...
                          {question.questionType === "speaking" && (
                            <div>
                              <p className="text-sm text-muted-foreground">
                                Speaking question - Students will record an audio response.
                              </p>
                            </div>
                          )}
                          ...
                          {question.questionType === "short_answer" && (
                            <div>
                              <p className="font-medium">Expected Answer: {Array.isArray(question.correctAnswer) 
                                ? question.correctAnswer.join(", ") 
                                : question.correctAnswer}
                              </p>
                            </div>
                          ...
                          <div className="mt-4 flex items-center justify-between">
                            <div>
                              <span className="text-sm font-medium">Marks: {question.marks}</span>
                              {question.timeLimit && (
                                <span className="text-sm font-medium ml-4">Time Limit: {question.timeLimit} seconds</span>
                              )}
                            </div>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      ...
                                      question and all associated student responses.
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
            ...
            <CardFooter className="flex justify-between">
              <div>
                <span className="text-sm text-muted-foreground">
                  Total {activeTab.split("_").map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ")} Questions: {filteredQuestions().length}
                </span>
              </div>
              <Button 
                onClick={() => router.push(`/mod/mock-tests/${params.id}/questions/create?type=${activeTab}`)}
