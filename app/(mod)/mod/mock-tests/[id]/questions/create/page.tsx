...
import { getMockTestById, createQuestion } from "@/controllers/MockTestController";
import { MockTest, Question, QuestionType, QuestionOption } from "@/lib/types/mock-test";
...
const baseQuestionSchema = z.object({
  ...
  questionType: z.enum(["multiple_choice", "fill_blank", "essay", "speaking", "short_answer"]),
...
export default function CreateQuestionPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    if (typeParam && ["multiple_choice", "fill_blank", "essay", "speaking", "short_answer"].includes(typeParam)) {
  ...
  useEffect(() => {
    form.reset({
      ...
      ...(questionType === "multiple_choice" && {
        options: fields.length > 0 ? fields : [
          { id: ID.unique(), text: "", isCorrect: false },
          { id: ID.unique(), text: "", isCorrect: false },
        ],
      }),
    ...
    <div className="container mx-auto py-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Create a new {questionType.split("_").map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ")} question for this test.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {(mockTest.category === "listening" || mockTest.category === "speaking") && (
                  <FormField
                    control={form.control}
                    name="audioUrl"
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Audio URL {mockTest.category === "listening" ? "(Required)" : "(Optional)"}</FormLabel>
                        <FormControl>
                          <div className="flex">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                // In a real app, you would implement file upload here
                                toast.info("Audio upload would be implemented here");
                              }}
                            >
                              <Upload className="h-4 w-4" />
                            </Button>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Time limit for answering this question (leave empty for no limit)
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
                    ...
                    <div key={field.id} className="flex items-start space-x-2">
                      <div className="flex-1 space-y-2 border p-4 rounded-md">
                        <div className="flex items-center justify-between">
                          <h4 className="font-medium">Option {String.fromCharCode(65 + index)}</h4>
                          <Button
                            type="button"
                            variant="ghost"
                            size="sm"
